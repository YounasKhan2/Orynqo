...
export function MyWorkPage() {
  ...
  const inspector =
    ...
    hasActiveFilter = Boolean(search.q) || search.status !== "not-done";
  return (
    <div className="flex h-[calc(100vh-48px)] min-h-[520px] flex-col overflow-hidden bg-[var(--surface-base)]">
      <header className="flex h-[74px] items-center justify-start border-b border-[var(--border-subtle)] px-3 max-sm:h-16">
        <div>
          <span className="text-[10px] font-semibold uppercase tracking-[.04em] text-[var(--text-tertiary)]">
            Personal retrieval
          </span>
          <h1 className="mt-0.5 mb-0 text-base leading-[1.2]">My Work</h1>
          <p className="mt-[3px] mb-0 text-[10px] text-[var(--text-secondary)]">
            Assigned to me · Product workspace
          </p>
        </div>
      </header>
      <MyWorkToolbar
        ...
        onGroup={(group) =>
          void navigate({ search: (p) => ({ ...p, group }), replace: true })
        }
        onSort={(sort) =>
          void navigate({ search: (p) => ({ ...p, sort }), replace: true })
        }
        onSearch={(q) =>
          void navigate({
            search: (p) => ({ ...p, q: q || undefined }),
            replace: true,
          })
        }
      />
      ...
      >
        {query.isLoading ? (
          ...
          <MyWorkCollection
            ...
            onReset={() =>
              void navigate({
                search: { status: "not-done", group: "due", sort: "priority" },
                replace: true,
              })
            }
          />

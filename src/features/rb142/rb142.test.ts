...
describe("RB-142 server-authoritative development contract", () => {
  ...
  it("executes the complete Golden Flow with server-owned versions and newest-first Activity", async () => {
    const priority = await rb142DevelopmentServer.execute(
      command("workItem.changePriority", 7, "m-priority", {
        priority: "urgent",
      }),
    );
    expect(priority.ok && priority.canonical.workItem).toMatchObject({
      priority: "urgent",
      version: 8,
    });
    const comment = await rb142DevelopmentServer.execute(
      command("workItem.createComment", 8, "m-comment", {
        body: RB142_COMMENT,
      }),
    );
    expect(comment.ok && comment.canonical.comments).toHaveLength(1);
    const status = await rb142DevelopmentServer.execute(
      command("workItem.transitionStatus", 9, "m-status", {
        statusId: "status-review",
      }),
    );
    expect(status.ok && status.canonical.workItem).toMatchObject({
      statusId: "status-review",
      statusLabel: "Review",
      version: 10,
    });
    ...
    expect(
      status.canonical.activity.slice(0, 3).map((event) => event.type),
    ).toEqual([
  ...
  it("rolls back logically on failure without Activity/version change", async () => {
    ...
    const result = await rb142DevelopmentServer.execute(
      command("workItem.changePriority", 7, "m1", { priority: "urgent" }),
    );
    expect(result).toMatchObject({ ok: false, code: "transport_error" });
    expect(rb142DevelopmentServer.read().workItem).toMatchObject({
      priority: "high",
      version: 7,
    });
  ...
  it("makes comment mutationId idempotent", async () => {
    const cmd = command("workItem.createComment", 7, "m-comment", {
      body: RB142_COMMENT,
    });
    ...
    expect(
      rb142DevelopmentServer
        .read()
        .activity.filter((e) => e.type === "workItem.comment.created"),
    ).toHaveLength(1);
  });

  it("rejects stale expectedVersion as conflict", async () => {
    const result = await rb142DevelopmentServer.execute(
      command("workItem.changePriority", 6, "m1", { priority: "urgent" }),
    );
  ...
  it("enforces the In Progress to Review workflow transition", async () => {
    const result = await rb142DevelopmentServer.execute(
      command("workItem.transitionStatus", 7, "m1", {
        statusId: "status-review",
      }),
    );
  ...
  it("represents archive and delete separately", () => {
    expect(rb142DevelopmentServer.remoteArchive().workItem.lifecycle).toBe(
      "archived",
    );
  ...
  it("applies an unrelated remote assignee update without touching Priority", () => {
    ...
    expect(snapshot.workItem).toMatchObject({
      priority: "high",

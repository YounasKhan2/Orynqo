...
export type RegressionMode =
  "none" | "failure" | "forbidden" | "conflict" | "archive" | "delete";
...
export class Rb142DevelopmentServer {
  ...
  private now() {
    return new Date(
      Date.parse(initialTime) + this.clock++ * 1000,
    ).toISOString();
  }
  ...
  async execute(command: Rb142Command): Promise<Rb142CommandResult> {
    ...
    if (this.mode === "delete" || this.snapshot.unavailable)
      return this.remember(command.mutationId, {
        ok: false,
        code: "not_found",
    ...
    if (
      ...
      return {
        ...
        code: "conflict",
        message:
          "RB-142 changed remotely. Review the canonical value before retrying.",
        canonical: this.read(),
      };
    ...
    if (command.commandType === "workItem.changePriority") {
      if (
        ...
        command.payload.priority !== "urgent"
      )
      ...
      activity = this.activity(
        command.mutationId,
        "workItem.priority.changed",
        { label: "Priority", from: previous, to: "urgent" },
        timestamp,
      );
    } else if (command.commandType === "workItem.createComment") {
      if (
        !("body" in command.payload) ||
        command.payload.body !== RB142_COMMENT
      )
        return {
          ok: false,
          code: "validation",
          message: "The RB-142 fixture comment must remain exact.",
        };
      ...
      activity = this.activity(
        ...
        { label: "Comment", body: command.payload.body },
        timestamp,
      );
    } else {
      if (
        !("statusId" in command.payload) ||
        command.payload.statusId !== "status-review"
      )
        return {
          ok: false,
          code: "validation",
      ...
      if (next.workItem.statusId !== "status-in-progress")
        return {
          ok: false,
          code: "validation",
          message: "Only In Progress → Review is valid in this slice.",
        };
      ...
      activity = this.activity(
        ...
        { label: "Status", from: "In Progress", to: "Review" },
        timestamp,
      );
    }
  ...
  remoteAssigneeChange() {
    return this.patchRemote({
      assigneeId: "user-remote",
      assigneeLabel: "N. Chen",
    });
  }
  remotePriorityChange(priority: "low" | "medium" | "high" | "urgent") {
    return this.patchRemote({ priority });
  }
  remoteArchive() {
    return this.patchRemote({ lifecycle: "archived" });
  }
  remoteDelete() {
    this.snapshot = {
      ...this.snapshot,
  ...
  private patchRemote(patch: Partial<WorkItemListItem>) {
    this.snapshot = {
      ...this.snapshot,
      workItem: {
        ...this.snapshot.workItem,
  ...
  private remember(id: string, result: Rb142CommandResult) {
    this.receipts.set(id, result);
    return result;
  }
  private can(type: Rb142Command["commandType"]) {
    if (type === "workItem.createComment") return this.snapshot.access.comment;
    if (type === "workItem.transitionStatus")
      return this.snapshot.access.changeStatus;

...
describe("RB-142 query projection reconciliation", () => {
  it("optimistically updates detail and collection without fabricating a version", () => {
    ...
    expect(
      client.getQueryData<WorkItemCollectionResult>(key)?.items[0],
    ).toMatchObject({ priority: "urgent", version: 7 });
  });
  it("rolls back every projection", () => {
    const client = new QueryClient();
    const { key } = seed(client);
    ...
    expect(
      client.getQueryData<WorkItemCollectionResult>(key)?.items[0]?.priority,
    ).toBe("high");
  });
  it("reconciles canonical status into both projections while Review remains not-Done", () => {
    const client = new QueryClient();
    const { key } = seed(client);
    const canonical = rb142DevelopmentServer.read();
    canonical.workItem.statusId = "status-review";
    canonical.workItem.statusLabel = "Review";
    ...
    expect(
      client.getQueryData<WorkItemCollectionResult>(key)?.items[0]?.statusLabel,
    ).toBe("Review");
    expect(
      client.getQueryData<WorkItemCollectionResult>(key)?.items,
    ).toHaveLength(1);
  });
  it("applies an unrelated canonical assignee while preserving pending Priority", () => {
    const client = new QueryClient();
    const { key } = seed(client);
    applyRb142OptimisticFields(client, { priority: "urgent" });
    const remote = rb142DevelopmentServer.remoteAssigneeChange();

...
export function Rb142Inspector({ onClose }: { onClose: () => void }) {
  ...
  return <aside className="work-inspector rb142-inspector" aria-labelledby="inspector-title">
    ...
    <section className="inspector-section" aria-labelledby="properties-title"><h3 id="properties-title">Properties</h3><div className="property-list">
      <div><span>Priority</span><button ref={priorityRef} type="button" disabled={readOnly || !snapshot.access.editProperties || slice.priorityState.phase === "pending"} onClick={() => setEditor(editor === "priority" ? null : "priority")} className="property-control capitalize">{priorityDisplay}</button><MutationStateLabel phase={slice.priorityState.phase}/></div>
      ...
      <div><span>Status</span><button ref={statusRef} type="button" disabled={readOnly || !snapshot.access.changeStatus || slice.statusState.phase === "pending"} onClick={() => setEditor(editor === "status" ? null : "status")} className="property-control">{statusDisplay}</button><MutationStateLabel phase={slice.statusState.phase}/></div>

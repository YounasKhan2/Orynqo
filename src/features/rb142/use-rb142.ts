...
export function useRb142(initial: Rb142Snapshot | undefined) {
  ...
  const run=useCallback(async(kind:"priority"|"status"|"comment")=>{
    const current=client.getQueryData<Rb142Snapshot>(queryKeys.workItem("wi-rb-142"))??initial;
    if(!current||current.unavailable)return;
    const id=nextId(kind);
    const args={mutationId:id,expectedVersion:current.workItem.version};
    const setState=kind==="priority"?setPriorityState:kind==="status"?setStatusState:setCommentState;
    const attemptedValue=kind==="priority"?"Urgent":kind==="status"?"Review":undefined;
    setState({phase:typeof navigator!=="undefined"&&!navigator.onLine?"offline":"pending",...(attemptedValue===undefined?{}:{attemptedValue})});
  ...
  const receiveRealtime=useCallback((event:Parameters<typeof rb142DevelopmentEvents.deliver>[0])=>{
    ...
    const pending=registry.forResource("wi-rb-142").filter(e=>e.phase==="pending"||e.phase==="offline");
  ...
  useEffect(()=>{
    const unsubscribe=rb142DevelopmentEvents.subscribe(receiveRealtime);
    return () => { unsubscribe(); };
  },[receiveRealtime]);

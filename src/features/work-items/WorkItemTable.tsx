...
export function WorkItemTable({
    ...
    () => [
      {
        ...
        cell: ({ row }) => (
          <input
            ...
            onChange={(event) =>
              onCheckedChange(row.original.id, event.currentTarget.checked)
            }
  ...
  return (
    <div
      className="work-table-frame"
      role="table"
      aria-label="Project Work Items"
      aria-rowcount={rows.length + 1}
    >
      <div
        className="work-table-header"
        role="row"
          ...
          {virtualizer.getVirtualItems().map((virtualRow) => {
            ...
            return (
              <div
                ...
                onKeyDown={(event) =>
                  onKeyDown(event, virtualRow.index, row.id)
                }
              >

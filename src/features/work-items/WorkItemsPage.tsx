...
export function WorkItemsPage() {
  ...
  const openInspector = (id: string) => {
    lastFocusedId.current = id; setSelectedWorkItemId(id);
  ...
  useEffect(() => {
    const handler = (event: globalThis.KeyboardEvent) => {
      ...
      event.preventDefault(); closeInspector();
    };
    window.addEventListener("keydown", handler);
    return () => window.removeEventListener("keydown", handler);
  });

  return <div className="work-items-page">
    <header className="project-context"><div><span className="eyebrow">Platform Core</span><h1>Issues</h1></div><button className="new-issue" type="button">New issue</button></header>
    ...
    <div className={search.selected ? "collection-layout inspector-open" : "collection-layout"}>
      <section className="collection-panel" aria-label="Project Work Items">
        {query.isLoading ? <TableSkeleton/> : query.isError ? <InlineAlert role="alert">Could not load Work Items. Your collection context is preserved.</InlineAlert> : query.data?.items.length === 0 ? <div className="collection-empty"><strong>No Work Items</strong><span>Adjust the current search or filter.</span></div> : query.data ? <>
          ...
          <WorkItemTable items={query.data.items} selectedWorkItemId={selectedWorkItemId} checkedWorkItemIds={checkedWorkItemIds} onContextSelect={setSelectedWorkItemId} onCheckedChange={setChecked} onOpen={openInspector}/>
        </> : null}
      </section>
...
function TableSkeleton(){return <div className="table-skeleton" aria-label="Loading Work Items">{Array.from({length:10},(_,i)=><div key={i} className="skeleton-row"><span/><span/><span/><span/></div>)}</div>}

...
export function WorkItemsPage() {
  ...
  const [selectedWorkItemId, setSelectedWorkItemId] = useState<
    string | undefined
  >(search.selected);
  const [checkedWorkItemIds, setCheckedWorkItemIds] = useState<Set<string>>(
    () => new Set(),
  );
  ...
  return (
    <div className="work-items-page">
      <header className="project-context">
        <div>
          <span className="eyebrow">Platform Core</span>
          <h1>Issues</h1>
        </div>
        <button className="new-issue" type="button">
          New issue
        </button>
      </header>
        ...
        <button
          ...
          onClick={() =>
            void navigate({
              search: (previous) => ({
                ...previous,
                status: previous.status === "all" ? "not-done" : "all",
              }),
              replace: true,
            })
          }
        ...
        <select
          ...
          onChange={(event) =>
            void navigate({
              search: (previous) => ({
                ...previous,
                sort: event.target.value as "key" | "title" | "priority",
              }),
              replace: true,
            })
          }
      ...
      <div
        className={
          search.selected
            ? "collection-layout inspector-open"
            : "collection-layout"
        }
      >
        <section className="collection-panel" aria-label="Project Work Items">
          {query.isLoading ? (
            ...
            <InlineAlert role="alert">
              Could not load Work Items. Your collection context is preserved.
            </InlineAlert>
          ) : query.data?.items.length === 0 ? (
            <div className="collection-empty">
              <strong>No Work Items</strong>
              <span>Adjust the current search or filter.</span>
            </div>
          ) : query.data ? (
            <>
              {query.data.stale && (
...
function TableSkeleton() {
  return (
    <div className="table-skeleton" aria-label="Loading Work Items">

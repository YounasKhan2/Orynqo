...
function NotificationRow({
  ...
  const safe = safeNotificationPresentation(notification);
  const unread = notification.readAt === null;
  const openable = canOpenNotificationTarget(notification);
  const onKeyDown = (event: KeyboardEvent<HTMLDivElement>) => {
    if (!openable || !isNotificationActivationKey(event.key)) return;
  ...
  return (
    <div
      ...
      tabIndex={notificationRowTabIndex(notification)}
    ...
    >
      <span
        ...
        className="grid size-4 place-items-center text-caption text-[var(--accent-default)]"
      >
        {unread ? "●" : "○"}
      </span>
      <span className="min-w-0">
        <strong className="block truncate text-ui text-[var(--text-primary)]">
          {safe.eventLabel}
        </strong>
        <span className="block truncate text-caption text-[var(--text-secondary)]">
          {safe.actorLabel} {safe.sourceEntityLabel} ·{" "}
          {formatNotificationAge(notification.occurredAt, now)}
        </span>
      </span>
      <button
        type="button"
        ...
        aria-label={
          unread ? "Mark notification as read" : "Mark notification as unread"
        }
      >
        ...
        <section key={group.id} aria-labelledby={`inbox-group-${group.id}`}>
          <header className="sticky top-0 z-[2] flex h-[30px] items-center gap-2 border-b border-[var(--border-subtle)] bg-[var(--surface-subtle)] px-3">
            <h2
              ...
              className="m-0 text-label text-[var(--text-secondary)]"
            >
              {group.label}
            </h2>
...
export function InboxEmpty() {
  return (
    <div className="grid h-full place-content-center gap-1 text-center text-caption text-[var(--text-secondary)]">
      <strong>No notifications yet</strong>
      <span>Personal notifications for this workspace will appear here.</span>
    </div>
  );
}
...
export function InboxLoading() {
  return (
    <div aria-label="Loading Inbox">
      {Array.from({ length: 7 }, (_, index) => (
        <div
          key={index}
          className="grid h-[52px] grid-cols-[18px_1fr_72px] items-center gap-2 border-b border-[var(--border-subtle)] px-3"
        >
          {Array.from({ length: 3 }, (_, part) => (
            <span
              key={part}
              className="h-2 rounded bg-[var(--surface-subtle)]"
            />
          ))}
        </div>
      ))}
...
export function InboxFailure() {
  return (
    <div
      role="alert"
      className="m-3 rounded-[var(--radius-sm)] border border-[var(--border-subtle)] bg-[var(--surface-subtle)] p-3 text-caption text-[var(--text-secondary)]"
    >
      Could not load Inbox. Your current context is preserved.
    </div>
  );
}
